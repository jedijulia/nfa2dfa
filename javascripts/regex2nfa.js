...
RegexParser.parse = function(regex, alphabet) {
    ...
    for (var i = 0; i < tokens.length; i++) {
        ...
        if (token.type === 'symbol') {
            if (alphabet.indexOf(token.content) >= 0) {
                ...
                nfa.getStartState().transition(state.finalize(),
                    token.content);
                concatStack.push(nfa);
            } else if (token.content === '*') {
                nfa = new NFA('ab');
                var popped = concatStack.pop();
                newStates = nfa.absorb(popped);
                nfa.getStartState().transition(
                    newStates[popped.getStartState().label], '~');
            ...
            nfa = RegexParser.parse(token.content, alphabet);
            concatStack.push(nfa);
        }
    }
    ...
    if (unionStack.length) {
        ...
        while (unionStack.length) {
            ...
            newStates = nfa.absorb(shifted);
            nfa.getStartState().transition(
                newStates[shifted.getStartState().label], '~');
        }
    }
    return nfa;
};
    ...
    for (var i = 0; i < regex.length; i++) {
        ...
        if (symbol === '(') {
            ...
            if (!stack.length) {
                tokens.push({
                    type: 'regex',
                    content: regex.substring(open + 1, i)
                });
            }
        } else {
            if (!stack.length) {
    ...
    for (var i = 0; i < regex.length; i++) {
        ...
        if (character === '+') {
            if (!i || i === regex.length - 1) {
                ...
                if ((alphabet.indexOf(nextChar) < 0 && nextChar !== '*')
                || nextChar === ')' || nextChar === '+'
                || (alphabet.indexOf(prevChar) < 0 && prevChar !== '*')
                || prevChar === '(' || prevChar === '+') {
                    return false;
                }
            }
...
RegexParser.clean = function(regex) {
    ...
    for (var i = 0; i < regex.length; i++) {
        var character = regex.charAt(i);
        if (!(character === '*'
        && (finalRegex.charAt(finalRegex.length - 1) === '*'
        || finalRegex === ''))) {
...
NFA.prototype.removeState = function(label) {
    if (label in this.states) {
        ...
        for (var l in this.states) {
            if (this.states.hasOwnProperty(l)) {
...
NFA.prototype.concatenate = function(nfa) {
    ...
    for (var i = 0; i < finalStates.length; i++) {
        finalStates[i].unfinalize().transition(
            newStates[nfa.getStartState().label], '~');
    }
    return newStates;
};
...
NFA.prototype.absorb = function(nfa) {
    var newStates = {}, label;
    for (label in nfa.states) {
        if (nfa.states.hasOwnProperty(label)) {
        ...
        if (nfa.states.hasOwnProperty(label)) {
            ...
            for (var symbol in state.transitions) {
                if (state.transitions.hasOwnProperty(symbol)) {
                    for (var i = 0; i < state.transitions[symbol].length;
                    i++) {
                        newStates[label].transition(
                            newStates[state.transitions[symbol][i].label],
                            symbol);
                    }
                }
            }
