...
NFAConverter.expandStringTransitions = function(nfa) {
    for (var state in nfa.states) {
        if (nfa.states.hasOwnProperty(state)) {
            for (var transition in nfa.states[state].transitions) {
                if (transition.length !== 1) {
                    for (var i = 0; i < nfa.states[state]
                            .transitions[transition].length; i++) {
                        ...
                        for (var j = 0; j < transition.length - 1; j++) {
                            ...
                            stringTransition = stringTransition.substring(1,
                                stringTransition.length);
                            currentState.transition(newState, first);
                            currentState = newState;
                        }
                        currentState.transition(nfa.states[state]
                            .transitions[transition][i], stringTransition);
                    }
    ...
    if ('~' in state.transitions) {
        ...
        for (var i = 0; i < state.transitions['~'].length; i++) {
            if (eStates.indexOf(state.transitions['~'][i].label) === -1) {
                moreEStates = NFAConverter.eClosure(nfa,
                    state.transitions['~'][i], eStates);
            }
...
NFAConverter.convert = function(nfa) {
    ...
    var qZeroEClosures = NFAConverter.eClosure(nfa,
        nfa.states.q0, []).sort();
    var deadState = {
        label: 'deadState',
        transitions: {},
    ...
    generatedStates.push({
        label: 'q0',
        transitions: {},
        containedStates: qZeroEClosures
    });
        ...
        for (var i = 0; i < alphabet.length; i++) {
            ...
            for (var j = 0; j < currState.containedStates.length; j++) {
                ...
                if (currSymbol in nfa.states[currContainedState].transitions) {
                    var trans = nfa.states[currContainedState]
                        .transitions[currSymbol];
            ...
            for (var l = 0; l < statesReached.length; l++) {
                var eClosure = NFAConverter.eClosure(nfa,
                    statesReached[l], []);
            ...
            generatedState = {
                label : 'q' + labelCtr,
                transitions: {},
                containedStates : eClosuresUnion
            };
            generatedState.transitions[currSymbol] = '';
            labelCtr++;
            if (contains(finalDFAstates, generatedState)) {
                currState.transitions[currSymbol]
                    = getLabel(finalDFAstates, generatedState);
                labelCtr--;
            } else {
                if (contains(generatedStates, generatedState)) {
                    currState.transitions[currSymbol]
                        = getLabel(generatedStates, generatedState);
                    labelCtr--;
                } else {
    ...
    for (var i = 0; i < finalDFAstates.length; i++) {
        ...
        for (var symbol in unConvState.transitions) {
            if (unConvState.transitions.hasOwnProperty(symbol)) {
                convState.transition(convertedStates[unConvState
...
function setFinalStates(finalDFAstates, finalNFAstates) {
    for (var i = 0; i < finalDFAstates.length; i++) {
        for (var j = 0; j < finalNFAstates.length; j++) {
            for (var k = 0; k < finalDFAstates[i].containedStates.length;
                    k++) {
                if (finalDFAstates[i].containedStates[k]
                === finalNFAstates[j].label) {
